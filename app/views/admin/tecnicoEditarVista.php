    ...
    <div class="bg-white p-6 rounded-xl shadow-md border border-gray-100">
        ...
        <div class="mb-6 border-b pb-4">
            <h1 class="text-2xl font-bold text-gray-800">
                <i class="fas fa-user-edit text-yellow-500 mr-2"></i> Editar Técnico
            </h1>
            <p class="text-gray-500 mt-1">Modificando: <strong><?= htmlspecialchars($datos['nombre_tecnico']) ?></strong></p>
        </div>
        ...
        <form action="" method="POST" class="space-y-6">
            <input type="hidden" name="id_tecnico" value="<?= $id ?>">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <select name="estado" class="pl-10 mt-1 block w-full px-3 py-3 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-yellow-500 bg-white">
                        <option value="1" <?= $datos['estado'] == 1 ? 'selected' : '' ?>>Activo (Disponible)</option>
                        <option value="0" <?= $datos['estado'] == 0 ? 'selected' : '' ?>>Inactivo (No Disponible)</option>
                    </select>
                </div>
            </div>

            <div class="pt-6 border-t border-gray-100 flex justify-end space-x-4">
                <a href="<?= BASE_URL ?>tecnicoVer" class="px-6 py-3 bg-white text-gray-700 font-semibold rounded-lg border border-gray-300 hover:bg-gray-50">Cancelar</a>
