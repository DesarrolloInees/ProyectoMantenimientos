...
class PuntoCrearModelo
{
    ...
    public function obtenerClientes() { return $this->conn->query("SELECT id_cliente, nombre_cliente FROM cliente WHERE estado = 1 ORDER BY nombre_cliente ASC")->fetchAll(PDO::FETCH_ASSOC); }
    public function obtenerMunicipios() { return $this->conn->query("SELECT id_municipio, nombre_municipio FROM municipio ORDER BY nombre_municipio ASC")->fetchAll(PDO::FETCH_ASSOC); }
    public function obtenerDelegaciones() { return $this->conn->query("SELECT id_delegacion, nombre_delegacion FROM delegacion WHERE estado = 1 ORDER BY nombre_delegacion ASC")->fetchAll(PDO::FETCH_ASSOC); }
    public function obtenerModalidades() { return $this->conn->query("SELECT id_modalidad, nombre_modalidad FROM modalidad_operativa ORDER BY nombre_modalidad ASC")->fetchAll(PDO::FETCH_ASSOC); }
}