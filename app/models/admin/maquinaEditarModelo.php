...
class MaquinaEditarModelo
{
    ...
    {
        return $this->conn->query("SELECT p.id_punto, p.nombre_punto, c.nombre_cliente FROM punto p INNER JOIN cliente c ON p.id_cliente = c.id_cliente WHERE p.estado = 1 ORDER BY p.nombre_punto ASC")->fetchAll(PDO::FETCH_ASSOC);
    }
