...
class PuntoEditarModelo
{
    ...
    public function obtenerClientes() { return $this->conn->query("SELECT id_cliente, nombre_cliente FROM cliente WHERE estado = 1")->fetchAll(PDO::FETCH_ASSOC); }
    public function obtenerMunicipios() { return $this->conn->query("SELECT id_municipio, nombre_municipio FROM municipio")->fetchAll(PDO::FETCH_ASSOC); }
    public function obtenerDelegaciones() { return $this->conn->query("SELECT id_delegacion, nombre_delegacion FROM delegacion WHERE estado = 1")->fetchAll(PDO::FETCH_ASSOC); }
    public function obtenerModalidades() { return $this->conn->query("SELECT id_modalidad, nombre_modalidad FROM modalidad_operativa")->fetchAll(PDO::FETCH_ASSOC); }
}