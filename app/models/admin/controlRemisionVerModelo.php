...
class ControlRemisionVerModelo
{
    ...
    public function contarTotalRemisiones($busqueda = null)
    {
        try {
            ...
            // Verificar si hay búsqueda y no está vacía
            if (!empty($busqueda) && trim($busqueda) !== '') {
                $sql .= " AND (cr.numero_remision LIKE :busqueda1 
                            OR t.nombre_tecnico LIKE :busqueda2 
                            OR e.nombre_estado LIKE :busqueda3)";
            ...
            // Solo bindear si hay parámetros
            ...
            error_log("Error en contarTotalRemisiones: " . $e->getMessage());
            return 0;
        }
    }
    ...
    public function obtenerRemisionesParaDatatable($start, $length, $columnaOrden, $direccionOrden, $busqueda = null)
    {
        try {
            ...
            // Bind de los parámetros de búsqueda (solo si hay)
            foreach ($params as $key => $value) {
                $stmt->bindValue($key, $value, PDO::PARAM_STR);
            }

            // Bind de límite y offset
            $stmt->bindValue(':limite', $length, PDO::PARAM_INT);
            $stmt->bindValue(':offset', $start, PDO::PARAM_INT);

            $stmt->execute();
            return $stmt->fetchAll(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
            error_log("Error en obtenerRemisionesParaDatatable: " . $e->getMessage());
            return [];
        }
    }
